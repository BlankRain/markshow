...
var gloves=require('./gloves-js/index.js');
...
var gitHubWebHandler = require('github-webhook-handler');
var app;
module.exports.start = function(port, pptDir, host, argvObj) {
    port = parseInt(port, 10) || 8080;
    pptDir = (pptDir || path.join(__dirname, '../markshowppts')) + path.sep;
    try {
    ...
    io.of('/touchevent').on('connection',function (socket){
        touchSockets[socket.id]=socket;
        gloves.fiveKeys.forEach((key)=>{
            gloves.createOnclickStream(key).stream.subscribe((x)=>{ 
                socket.emit('clickdata',{k:key,d:x});
            });
        });
...
function startApp(port, dir, host, argvObj) {
    ...
    //for github web hook
    var gitHander = gitHubWebHandler({
        path: '/githubwebhook',
        secret: argvObj['webhooksecret'] || 'lalalalala'
    });
    gitHander.on('error', function(err) {
        console.error('Error:', err.message)
    });
    gitHander.on('push', function(event) {
        console.log('Received a push event for %s to %s',
            event.payload.repository.name,
            event.payload.ref);
        var pullCmd = 'cd ' + pptDir + ' && git pull';// should I config it as a config?
        console.log(pullCmd);

        exec(pullCmd, function(err, stdout, stderr) {
            if (err) {
                console.log(err);
            }
