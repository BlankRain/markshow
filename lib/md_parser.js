...
var parser = function (string, callback, argvObj, queryObj, commonData) {
    ...
    contents.forEach(function (v, i) {
        ...
        var attrs = '';
        if (cfg) {
            attrs = cfg[1]||cfg[2];// and chinese support
        }
...
///test
// var str = fs.readFileSync('markshowppts/demo.md').toString();
...
中文属性语法支持
**/
function zhAttrDSLSupport(attrStr){
  var zhMap={
'左右翻页':'kontext',
'翻页':'kontext',
'上下翻页':'vkontext',
...
'缩入':'zoomIn'
  ...
  for(var ek in zhMap){
    if(ek!=''){
    attrStr=attrStr.replace(new RegExp(ek,'gm'),zhMap[ek]);
    }
  }
  return attrStr;
}
