...
function SnakeJS(parentElement, config){
	...
	var defaultConfig = {
		...
		gridWidth : 60,						// Width of the game grid
	...
	function InputInterface(pauseFn, resumeFn, autoPlayFn){
		var arrowKeys=[Control.top,Control.left,Control.bottom,Control.right],// wasd
		...
		this.startListening = function(){
			if (!listening) {
				...
				$(window).on("jq-keydown",(event,data)=>{
					with (constants) {
						switch (data.keyCode) {
						case Control.left:
